...
```{r Ex_4}
m = as.numeric(Ex3_cont_obj[["page"]][["totalElements"]])  # Number total elements
n = as.numeric(count(Ex3))  # Number of elements per page

pages <- floor(m/n)  # 237 (13/02/2021)
# Controlling for remainder: 
remainder <- m - pages*n  # 3 -> eventually need to adjust for last page
...
Ex4_9 <- all_pages(country, 9, df_Ex4)  # Woeks sometimes
Ex4_10 <- all_pages(country, 10, df_Ex4)  # Works sometimes
# location does not exist
# Error: no applicable method for 'select_' applied to an object of class "NULL"

# Troubleshooting - single page in detail
test_page10_glimpse <- first_glimpse(country, 10)  # works
test_page10 <- get_content(country, 10)  # location does not exist -> Error
...
Ex4_15 <- all_pages(country, 15, df_Ex4)  # works
