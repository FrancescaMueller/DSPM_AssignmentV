...
get_content <- function(country, page) {
  ...
  cont_obj <- as.data.frame(cont_obj$`_embedded`$venues)  # HERE -> some observations have no locations - BUT error if a whole page does not have variable location!
  
  # Just keep relevant information asked by the assignment
  df <- data.frame(cont_obj$name, cont_obj$city[1],
